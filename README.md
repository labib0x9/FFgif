...
Migrating the codebase from a flat layered architecture to **Domain-Driven Design (DDD)**:

```
internal/
  domain/auth/       entity, repository interface, sentinel errors
  app/auth/          use cases: signup, login, logout, verify, resend, forgot/reset password
  infra/postgres/    concrete repository implementations
  transport/http/    thin HTTP adapter layer, routes
```

The goal is cleaner separation of business logic from infrastructure, making the codebase easier to test and extend. The auth domain is the initial target.
