...
func (h *Handler) GetProfile(w http.ResponseWriter, r *http.Request) {
	...
	if id == "" {
		...
		slog.Error("user handler - GetProfile() = user_id not found", "request_id", reqId, "err", "user_id not found")
		return
	}
	...
	if err != nil {
		...
		slog.Error("user handler - GetProfile()", "request_id", reqId, "err", err)
		return
	}

	etag := fmt.Sprintf(`"%s"`, found.UpdatedAt.Format(time.RFC3339Nano))
