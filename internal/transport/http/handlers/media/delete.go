...
func (h *Handler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	if id == "" {
		...
		slog.Error("media handler - Delete() = user_id not found", "request_id", reqId, "err", "user_id not found")
		return
	}
	...
	if key == "" {
		...
		slog.Warn("media handler - Delete() = gif key missing", "request_id", reqId, "error", "gif key missing")
		return
	}
	...
	if err := h.srv.Delete(r.Context(), id, key); err != nil {
		...
		slog.Error("media handler - Delete()", "request_id", reqId, "err", err)
		return
	}
